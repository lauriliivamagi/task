...
export class EmbeddingService {
  ...
  private async init(): Promise<boolean> {
    ...
    await this.ensureVectorSpace();

    return true;
  }

  /**
   * Guard against silently mixing vector spaces: embeddings generated by
   * different providers/models are not comparable, so a provider switch must
   * clear the stored vectors (they're fully rebuildable via backfill) rather
   * than poison search rankings.
   */
  private async ensureVectorSpace(): Promise<void> {
    ...
    const spaceId = this.provider.spaceId;

    const meta = await this.db.execute(
      "SELECT value FROM emb.embedding_meta WHERE key = 'space'",
    );
    const storedSpace = meta.rows[0]?.value as string | undefined;

    if (storedSpace === spaceId) {
      return;
    }

    if (storedSpace !== undefined) {
      // Provider/model changed - drop the now-incomparable vectors
      ...
      if (total > 0) {
        await this.db.execute("DELETE FROM emb.task_embeddings");
        await this.db.execute("DELETE FROM emb.comment_embeddings");
        logger.warn(
          `Embedding provider changed (${storedSpace} -> ${spaceId}); ` +
            `cleared ${total} stale vectors. Run 'task embeddings backfill' to rebuild.`,
          "embeddings",
        );
        console.error(
          `Embedding provider changed (${storedSpace} -> ${spaceId}). ` +
            `Cleared ${total} stale vectors — run 'task embeddings backfill' to rebuild the search index.`,
        );
      }
    }

    await this.db.execute({
      sql: `INSERT INTO emb.embedding_meta (key, value) VALUES ('space', ?)
